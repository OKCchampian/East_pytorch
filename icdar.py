...
def get_images(root):
    ...
    for ext in ['jpg', 'png', 'jpeg', 'JPG']:
        files.extend(gb.glob(
            os.path.join(root, '*.{}'.format(ext))))
    name = []
    for i in range(len(files)):
        name.append(files[i].split('/')[-1])

    print('Total:{} imgs for train'.format(len(files)))
    files = sorted(files)
    name = sorted(name)
    return files, name
...
def load_annoataion(p):
    ...
    with codecs.open(p, 'r', 'utf-8') as fin:
        for line in fin.readlines():
            ...
            x1, y1, x2, y2, x3, y3, x4, y4 = list(map(float, line_1[:8]))
